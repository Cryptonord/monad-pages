...
const CONTRACT_ADDRESS = "0xe44CC418d9f492a3bbe58CCE2b43a490d0E54A1b";

const ABI = [
  "function registerSession(string _sessionPubKey) external",
...
export default function Home() {
  ...
  const connectMain = async () => {
    ...
    // Safety Check: Warn if not on Monad Mainnet (Chain ID 143)
    const network = await provider.getNetwork();
    if (network.chainId !== 143n) {
      console.log("Warning: Not on Monad Mainnet");
    }
  ...
  const registerOnChain = async () => {
    ...
    try {
      ...
      const tx = await contract.registerSession(pubKey);
      await tx.wait();
      setStatus("Registered! You are online.");
    } catch (e: any) {
      console.error(e);
  ...
  const sendMessage = async () => {
    if (!burnerWallet || !recipient) return;
    setStatus("Sending...");
    ...
    try {
      ...
      if (!recipientPubKey) return alert("User hasn't registered a session yet!");
      ...
      const tx = await contract.sendMessage(recipient, cipherString, { gasLimit: 500000 });
      await tx.wait();
      ...
      console.error(e);
      setStatus("Send Failed. Check Balance/Gas!");
    }
  };
